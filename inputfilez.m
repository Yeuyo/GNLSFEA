function [coord,etpl,fext,bc,lstps,NRitmax,NRtol,E,nu,ka]=inputfilez
...
coord(:,6)=dep;
for i=1:(nels*2+1)
  coord((i-1)*3+1,2:3)=[0 (i-1)*len/(2*nels)];  
  coord((i-1)*3+2,2:3)=[wid/2 (i-1)*len/(2*nels)];
  coord((i-1)*3+3,2:3)=[wid (i-1)*len/(2*nels)];
end
for i=1:nels
  n1=(i-1)*6+1;
  n2=n1+1;
...
%   etpl(i,:)=[n1 n2 n3 n4 n5 n6 n7 n8 n9];
  etpl(i,:)=[n7 n6 n5 n4 n3 n2 n1 n8 n9];
end
nodes=size(coord,1);
fext=zeros(nodes*ndim,1);
fext( nodes*5   -4)=4*1/6;
fext((nodes-1)*5-4)=4*2/3;
fext((nodes-2)*5-4)=4*1/6;
